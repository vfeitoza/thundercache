...
int ProxyHandler::CommunicationHTTP()
{
    ...
    int64_t ContentLength = BodyLength;
...
int ProxyHandler::CommunicationSSL()
{
    ...
    if ( UseParentProxy )
    {
        if ( ToServer.SetDomainAndPort( ParentHost, ParentPort, ToBrowser.GetRequest() ) == false )
        {
            LogFile::ErrorMessage("Could not resolve parent proxy (%s)\n", ParentHost.c_str() );
            return -51;
        }
        ...
        if ( ToServer.SendHeader( Header, true, ToBrowser.GetRequest() ) == false )
        {
        ...
        if ( Header.substr(Position, 3) != "200" )
        {
            ...
            int64_t ContentLengthReference = ToServer.GetContentLength();
...
bool ProxyHandler::ProxyMessage( int CommunicationAnswerT, string Answer )
{
    ...
    if ( HeaderSend == false )
    {
        ...
        {
            errorheader += "DNS error by Thunder";
        }
        else if ( filename == "ERROR DOWN" )
        {
            errorheader += "Server down by Thunder";
        }
        else if ( filename == "ERROR REQUEST" )
        {
            errorheader += "Request error by Thunder";
        }
        else
        {
            errorheader += "Forbidden by Thunder";
        }
